...
@Meryem (AI): This app is your domain — it does not touch `api/models.py`, so it won't conflict
with Fati's schema work. It currently has its own `Document` model (self-contained; not yet
linked to a real `Invoice`, since that model doesn't exist yet).
